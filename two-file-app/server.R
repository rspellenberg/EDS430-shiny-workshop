...
server <- function(input, output){
  ...
  trout_filtered_df <- reactive({
    ...
    validate(
      need(length(input$channel_type_input) > 0, "Please select one or more channel type(s) to visualize data :)"),
  ...
  alt = "This is my alt text and it's not very good right now, but we will update later"
  
  ) # END of trout scatterplot
  ...
  }) # END of penguin histogram
    
  # penguin histogram ----
  output$penguin_histogram_output <- renderPlot({
    
    ggplot(na.omit(island_df()), aes(x = flipper_length_mm, fill = species)) +
      geom_histogram(alpha = 0.6, position = "identity", bins = input$penguin_bins_input) +
